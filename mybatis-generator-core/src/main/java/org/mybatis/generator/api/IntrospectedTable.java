...
public abstract class IntrospectedTable {
     ...
     * @return
     */
    public List<IntrospectedColumn> getSomeBaseColumns() {

        List<IntrospectedColumn> answer = new ArrayList<IntrospectedColumn>();
        answer.addAll(baseColumns);

        List<IntrospectedColumn> createAndUpdateTime = new ArrayList<>();
        for(IntrospectedColumn c: answer){
            if("create_time".equals(c.getActualColumnName())){
                createAndUpdateTime.add(c);
            }
            if("update_time".equals(c.getActualColumnName())){
                createAndUpdateTime.add(c);
            }
        }
        answer.removeAll(createAndUpdateTime);
     ...
     * @return
     */
    public List<IntrospectedColumn> getSomeColumns() {
        List<IntrospectedColumn> answer = new ArrayList<IntrospectedColumn>();
        answer.addAll(primaryKeyColumns);
        answer.addAll(baseColumns);
        answer.addAll(blobColumns);
     ...
     * @return
     */
    public List<IntrospectedColumn> getSomeNonPrimaryKeyColumns() {
        List<IntrospectedColumn> answer = new ArrayList<IntrospectedColumn>();
        answer.addAll(baseColumns);
        answer.addAll(blobColumns);
    ...
    public String getMapType() {
        return internalAttributes.get(InternalAttribute.ATTR_MAP_TYPE);
    }
